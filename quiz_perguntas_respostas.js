...
console.log ('----- QUIZ DE FATOS HISTÓRICOS -----')
 console.log('Seja Bem-vindo jogador(a)!', nome)
...
const questoes = [
  ...
  function selecionarQuestoesAleatorias (){
    return Math.floor(Math.random() * questoes.length)
  }

  function exibirPergunta(questao){
    console.log(questao)
  }

  function validarRespostaUsuario(resposta){

    const respostaUsuario = entraDados.question('Digite a resposta: ')
  ...
  while (numeroDePerguntas < 10){
    indice = selecionarQuestoesAleatorias()
    pergunta = questoes[indice]
    questoes.splice(indice, 1)
    exibirPergunta(pergunta.pergunta)
    validarRespostaUsuario(pergunta.resposta)
    numeroDePerguntas++
  }


  function exibirResultado (){
    if (numeroDeRespostasCorretas <= 3){
        console.log('OH NÃO! Tente mais uma vez', nome, 'voce acertou: ', numeroDeRespostasCorretas)

    } else if (numeroDeRespostasCorretas > 3 && numeroDeRespostasCorretas <= 6){
        console.log('BOM TRABALHO! Pratique um pouco mais', nome, 'voce acertou: ', numeroDeRespostasCorretas)

    } else if (numeroDeRespostasCorretas >=7 && numeroDeRespostasCorretas <=9){
        console.log ('MUITO BOM!,', nome, 'Você acertou a maioria',  numeroDeRespostasCorretas )

    } else if (numeroDeRespostasCorretas === 10){
        console.log('VOCE É UM EXECELNTE EXPERT!!!',  nome, 'acertou: ', numeroDeRespostasCorretas)
    }
  }
